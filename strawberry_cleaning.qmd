...
# Assuming your data frame is called `df` and the character column is called `column_name`
df <- df %>%
  mutate(column_name = case_when(
    column_name == "H" ~ 99.5,
    column_name == "L" ~ 0.05,
    column_name == "D" ~ NA_real_,
    TRUE ~ as.numeric(column_name)  # Convert numeric characters to numeric values
  ))

# Ensure the column is now numeric
...
strawberry <- strawberry %>%
  mutate(
    ...
    Fertilizer_Status = case_when(
      ...
      !grepl("\\d+\\.?\\d+", Category2) & !grepl("\\(NOP USDA CERTIFIED\\)", Category2) ~ Category2,  
      TRUE ~ NA_character_
    )
  ) %>%
  select(-Category2)
...
strawberry <- strawberry %>%
  ...
  separate(Category1, into = c("Chemical_Info", "Chemical_Product_Number"), sep = "=", extra = "merge") %>%
...
strawberry$Chemical_Product_Number <- str_trim(strawberry$Chemical_Product_Number, side = "both")
...
### Deal with NA values in Value and CV
```{r}
strawberry_alabama <- strawberry %>%
